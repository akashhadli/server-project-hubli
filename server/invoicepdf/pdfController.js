...
try {
  ...
  // const fileDownlaod = path.join(publicFolder, targetLocation);
  const fileDownlaod = 'https://drive.google.com/file/d/1dabEr-Uy7NrAwbGEdMTd_VppNXqxrpbD/view?usp=sharing';
  console.log(fileDownlaod);
  const imagePath = path.join(publicFolder, imageLocation, qrCodeFilename);
  const qrCodeData = JSON.stringify(fileDownlaod);
